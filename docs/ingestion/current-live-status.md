...
- live browser submit is wired through the web BFF to the DEV data-lake bridge when `ANNONA_DATA_LAKE_SUBMIT_URL` and `ANNONA_DATA_LAKE_STATUS_URL` are configured
- the live bridge accepts browser uploads, creates an intake/job identity, and exposes job polling through customer status
...
## Proof artifact disclosure

The available real successful submit screenshots are live DEV proof artifacts from the earlier `/?proof=1` workspace. They show a real accepted submit, the resulting dataset version, and downstream grounded use of the uploaded data.

The current `/ingestion`, `/datasets`, and `/customers` screenshots are Playwright/mock UX captures that validate the current route structure after the route split. Screenshots that display `Mocked DEV snapshot` or `Playwright submit mock` validate UX structure, not live bridge persistence.
...
If the web UI is running without `ANNONA_DATA_LAKE_STATUS_URL` and `ANNONA_DATA_LAKE_SUBMIT_URL`, the onboarding pages show deterministic DEV snapshot data and live upload submission is disabled outside Playwright test mode.
